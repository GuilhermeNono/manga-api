...
export const parseManga = (html:any, id:string) => {
    ...
    manga.description = series_desc_div.match(/(?<=<span class="series-desc">.*?span>).*?(?=<\/span>)/gm)[0].trim().replace(/<br>/gm, "").trim().replace(/<(\/|)(br|a|b|span)(\/|)>/gm, "").replace(/&nbsp;/gm, " ");
// series_desc_div.match(/(?<=<span class="series-desc">.*?span>).*?(?=<\/span>.*?<ol)/gm)[0].trim().replace(/<br>/gm, "").trim().replace(/<(\/|)(br|a|b|span)(\/|)>/gm, "").replace(/&nbsp;/gm, " ");
